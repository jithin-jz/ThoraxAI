  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-canvas border border-hairline rounded-md overflow-hidden divide-y divide-hairline">
        {tenants.map((t) => (
          <div key={t.hospital_id} className="px-4 sm:px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:bg-surface-card transition-colors">
            <div className="flex items-center gap-4 min-w-0">
              <div className="w-10 h-10 rounded-md bg-surface-card text-primary flex items-center justify-center shrink-0">
                <Building2 className="w-5 h-5" strokeWidth={1.5} />
              </div>
              <div className="min-w-0">
                <p className="font-bold text-ink tracking-tight truncate" title={t.name}>{t.name}</p>
                <p className="text-xs text-ash font-mono mt-0.5 truncate">Code: {t.invite_code}</p>
              </div>
            </div>
