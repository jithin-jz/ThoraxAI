    ...
    <div className="space-y-6">
      ...
      {filtered.length === 0 ? (
        ...
        <div className="bg-canvas border border-hairline rounded-md overflow-hidden divide-y divide-hairline">
          {filtered.map((p) => (
            <div key={p.patient_id} className="px-4 sm:px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:bg-surface-card transition-colors">
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-9 h-9 bg-surface-card rounded-full flex items-center justify-center text-ink text-xs font-bold shrink-0">
                  {p.name?.charAt(0).toUpperCase()}
                </div>
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-ink truncate" title={p.name}>{p.name}</p>
                  <p className="text-xs text-ash truncate">{p.age}y / {p.gender}</p>
                </div>
              </div>
              <div className="flex items-center gap-2 self-end sm:self-auto">
                <button onClick={() => navigate(`/dashboard/patients/${p.patient_id}`)} className="p-2 text-ash hover:text-ink hover:bg-surface-card rounded-md transition-colors cursor-pointer" title="View details"><Eye className="w-4 h-4" /></button>
                <button onClick={() => setConfirmDelete(p.patient_id)} className="p-2 text-ash hover:text-error hover:bg-red-50 rounded-md transition-colors cursor-pointer" title="Delete"><Trash2 className="w-4 h-4" /></button>
              </div>
            </div>
          ))}
        </div>
