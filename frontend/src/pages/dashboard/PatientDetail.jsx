...
export default function PatientDetail() {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Patient info */}
      <div className="bg-canvas border border-hairline rounded-md p-5 sm:p-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <button onClick={() => navigate(`/dashboard/scans?patient=${patientId}`)} className="w-full sm:w-auto flex items-center justify-center gap-2 px-4 h-10 bg-primary text-white rounded-md text-sm font-bold hover:bg-primary-pressed transition-colors cursor-pointer">
      ...
      {scans.length === 0 ? (
        ...
        <div className="bg-canvas border border-hairline rounded-md overflow-hidden divide-y divide-hairline">
          {scans.map((s) => {
            ...
            return (
              <div key={s.scan_id} className="px-4 sm:px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:bg-surface-card transition-colors cursor-pointer" onClick={() => navigate(`/dashboard/scans/${s.scan_id}`)}>
                <div className="flex items-center gap-4 min-w-0">
                  <div className="w-10 h-10 rounded-md bg-surface-card text-ash flex items-center justify-center shrink-0">
                    <ScanLine className="w-5 h-5" />
                  </div>
                  <div className="min-w-0">
                    <p className="font-semibold text-ink text-sm truncate">{s.scan_type?.replace("_", " ")} scan</p>
                    <p className="text-xs text-ash font-mono truncate">ID: {s.scan_id}</p>
                  </div>
                </div>
