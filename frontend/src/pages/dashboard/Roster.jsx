    ...
    <div className="space-y-8">
      ...
      {roster.length === 0 ? (
        ...
        <div className="bg-canvas border border-hairline rounded-md overflow-hidden divide-y divide-hairline">
          {roster.map((d) => (
            <div key={d.email} className="px-4 sm:px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:bg-surface-card transition-colors">
              <div className="flex items-center gap-4 min-w-0">
                <div className="w-10 h-10 rounded-full bg-surface-card text-ink flex items-center justify-center font-bold text-sm shrink-0">
                  {d.email?.charAt(0).toUpperCase()}
                </div>
                <div className="min-w-0">
                  <p className="font-semibold text-ink text-sm truncate" title={d.email}>{d.email}</p>
                  <p className="text-xs text-ash font-medium truncate">{d.role}</p>
                </div>
              </div>
