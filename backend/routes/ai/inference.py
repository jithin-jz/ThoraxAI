...
def _sync_run_inference(tensor: torch.Tensor, body_part_key: str) -> dict:
...
async def run_inference(tensor: torch.Tensor, body_part_key: str) -> dict:
    ...
    Offloaded to a background thread pool to prevent blocking the event loop.
    """
    loop = asyncio.get_running_loop()
    return await loop.run_in_executor(_executor, _sync_run_inference, tensor, body_part_key)
