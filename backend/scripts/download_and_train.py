...
def download():
    ...
    os.system(f"kaggle datasets download -d {KAGGLE_DATASET} " f"--path {DATA_DIR} --unzip")
...
def train():
    ...
    train_dl = DataLoader(
        train_ds, batch_size=BATCH, shuffle=True, num_workers=WORKERS, pin_memory=False
    )
    val_dl = DataLoader(
        val_ds, batch_size=BATCH, shuffle=False, num_workers=WORKERS, pin_memory=False
    )
    ...
    cached_weights = (
        _Path.home() / ".cache" / "torch" / "hub" / "checkpoints" / "densenet121-a639ec97.pth"
    )
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        print(
            ...
            f"val loss={v_loss/len(val_ds):.4f} acc={v_acc:.1f}%" + mark
        )
...
if __name__ == "__main__":
    ...
    already_organised = (TRAIN_DIR / "Normal").exists() and any((TRAIN_DIR / "Normal").iterdir())
    ...
    if already_organised:
        ...
        if hf_ok:
            ...
            if zip_file.exists() and zip_file.stat().st_size > 1_000_000_000:
                print(
                    f"📦 ZIP already downloaded ({zip_file.stat().st_size / 1e9:.2f} GB) — skipping download\n"
                )
