  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-canvas border border-hairline rounded-md overflow-hidden divide-y divide-hairline">
        {users.map((u) => (
          <div key={u.id} className="px-4 sm:px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:bg-surface-card transition-colors">
            <div className="flex items-center gap-4 min-w-0">
              <div className="w-9 h-9 rounded-full bg-surface-card text-ink flex items-center justify-center font-bold text-xs shrink-0">
                {u.email?.charAt(0).toUpperCase()}
              </div>
              <div className="min-w-0">
                <p className="font-semibold text-ink text-sm truncate" title={u.email}>{u.email}</p>
                <p className="text-xs text-ash font-mono mt-0.5 truncate">{u.hospital_id ? `Tenant: ${u.hospital_id}` : "No tenant"}</p>
              </div>
            </div>
