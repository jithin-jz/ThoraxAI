...
function safeTenantRedirect(url, path = "/dashboard", tokens = null) {
  if (url && isTrustedTenantUrl(url)) {
    let target = url.replace(/\/$/, "") + path;
    if (tokens) {
      const q = new URLSearchParams(tokens).toString();
      target += `?${q}`;
    }
...
export default function Auth() {
  ...
  useEffect(() => {
    ...
    const ssoPasskey = params.get("has_passkey") === "true";

    if (ssoToken) {
      login(ssoToken, ssoRefresh, ssoPasskey).then(() => {
        // Clean URL to hide tokens from history/address bar
  ...
  const handleRegisterPasskey = async () => {
    ...
    try {
      ...
      if (res.access_token) {
        ...
        const currentSub = getCurrentSubdomain();
        const targetSub = subdomain;
        if (currentSub === targetSub) {
          navigate("/dashboard");
        } else {
          if (safeTenantRedirect(tenantUrl, "/login", {
            token: res.access_token,
            refresh_token: res.refresh_token,
            has_passkey: "true"
          })) {
      ...
      setTimeout(() => {
        const tokens = {
          ...
          has_passkey: localStorage.getItem("has_passkey") || "false"
        };
        if (safeTenantRedirect(tenantUrl, "/login", tokens)) {
          clearTokens();
        } else {
          navigate("/dashboard");
        }
